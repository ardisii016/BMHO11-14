...
<head>
    ...
    <link rel="stylesheet" href="cancelapp-style.css">
              ...
              <div class="sales-details">
                <ul class="details">
                  ...
                  <?php
                    ...
                    $query = "SELECT * FROM appointment WHERE status = 'cancel' ORDER BY dateofreq - dateofapp";
                    $query_run = mysqli_query($conn, $query);

                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable"><a href='view-cancelapp.php?reference=<?php echo $items['reference']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &fullname=<?php echo $items['fullname']?>'> <?= ucwords($items['reference']) ?></a></li>
                        <?php
                      }
                    }
                  ?>
                ...
                <?php
                    ...
                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable"><a href='view-cancelapp.php?reference=<?php echo $items['reference']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &fullname=<?php echo $items['fullname']?>'> <?= ucwords($items['fullname']) ?></a></li>
                        <?php
                      }
                    }
                  ?>
              ...
              <ul class="details">
                ...
                <?php
                    ...
                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable"><a href='view-cancelapp.php?reference=<?php echo $items['reference']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &fullname=<?php echo $items['fullname']?>'> <?= ucwords($items['dateofapp']) ?></a></li>
                        <?php
                      }
                    }
                  ?>
...
<script src="cancelapp.js"></script>
