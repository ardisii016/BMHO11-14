...
<head>
    ...
    <link rel="stylesheet" href="doneapp-style.css">
    <title>Done Appointment</title>
      ...
      <section class="home-section">
        <nav>
          <div class="sidebar-button">
            ...
            <span class="dashboard">Done Appointment</span>
          </div>
        ...
        <div class="home-content">
          ...
          <div class="sales-boxes">
            <div class="recent-sales box">
              <div class="title">Done Appointment</div>
              <div class="sales-details">
                <ul class="details">
                  ...
                  <?php
                    ...
                    $query = "SELECT * FROM appointment WHERE status = 'done'  ORDER BY dateofreq - dateofapp";
                    $query_run = mysqli_query($conn, $query);

                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable"><a href='view-done.php?reference=<?php echo $items['reference']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &fullname=<?php echo $items['fullname']?>'> <?= ucwords($items['reference']) ?></a></li>
                        <?php
                      }
                    }
                  ?>
                ...
                <?php
                    ...
                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable"><a href='view-done.php?reference=<?php echo $items['reference']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &fullname=<?php echo $items['fullname']?>'> <?= ucwords($items['fullname']) ?></a></li>
                        <?php
                      }
                    }
                  ?>
              ...
              <ul class="details">
                ...
                <?php
                    ...
                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable"><a href='view-done.php?reference=<?php echo $items['reference']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &fullname=<?php echo $items['fullname']?>'> <?= ucwords($items['dateofapp']) ?></a></li>
                        <?php
                      }
                    }
                  ?>
