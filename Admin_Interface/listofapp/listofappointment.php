...
<head>
    ...
    <link rel="stylesheet" href="listofapp.css">
      ...
      <section class="home-section">
        <nav>
          <div class="sidebar-button">
            ...
            <span class="dashboard">Future Appointments</span>
          </div>
        ...
        <div class="home-content">
          ...
          <div class="sales-boxes">
            <div class="recent-sales box">
              <div class="title" >List of Future Appointment</div>
              <div class="sales-details" id="myTable">
                <ul class="details">
                  <li class="topic" >Reference no.</li>
                  <?php
                    include '../database/config.php';

                    $query = "SELECT * FROM appointment WHERE dateofapp != now && status = 'pending' ORDER BY dateofapp";
                    $query_run = mysqli_query($conn, $query);

                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li> <a href="view-futureapp.php?reference=<?php echo $items['reference']?> &fullname=<?php echo $items['fullname']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &now=<?php echo $items['now']?>"><?= ucwords($items['reference']) ?> </a></li>
                    <?php
                      }
                    }
                  ?>  
                ...
                <?php
                  ...
                  foreach($query_run as $items) {
                ?>
                <li class="pname"><a href="view-futureapp.php?reference=<?php echo $items['reference']?> &fullname=<?php echo $items['fullname']?> &typeofapp=<?php echo $items['typeofapp']?> &timeofapp=<?php echo $items['timeofapp']?> &dateofapp=<?php echo $items['dateofapp']?> &now=<?php echo $items['now']?>"><?= ucwords($items['fullname']) ?></a></li>
                <?php
                      }
                    }
                ?>
              ...
              <ul class="details">
                ...
                <?php
                  ...
                  foreach($query_run as $items) {
                ?>
                <li class="doa"><a href="" style='cursor: default;'><?= ucwords($items['dateofapp']) ?></a></li>
                <?php
                      }
                    }
                ?>
...
<script src="listofapp.js"></script>
