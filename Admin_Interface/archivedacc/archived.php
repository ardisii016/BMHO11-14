      ...
      <section class="home-section">
        <nav>
          <div class="sidebar-button">
            ...
            <span class="dashboard">Archived Account</span>
          </div>
        ...
        <div class="home-content">
          ...
          <div class="sales-boxes">
            <div class="recent-sales box">
              <div class="title">Archived Account</div>
              <div class="sales-details">
                ...
                <ul class="details">
                  ...
                  <?php
                    ...
                    $query = "SELECT * FROM `registration` WHERE usertype='removed'";
                    $query_run = mysqli_query($conn, $query);

                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable"><a href='view-archived.php?lastname=<?php echo $items['lastname']?> &firstname=<?php echo $items['firstname']?> &middlename=<?php echo $items['middlename']?> &gender=<?php echo $items['gender']?> &suffix=<?php echo $items['suffix']?> &dob=<?php echo $items['dob'] ?>&age=<?php echo $items['age']?> &contact=<?php echo $items['contact']?> &email=<?php echo $items['email']?> &address=<?php echo $items['address']?> &barangay=<?php echo $items['barangay']?> &img=<?php echo $items['img'] ?>'> <?= ucwords($items['user_id']) ?></a></li>
                        <?php
                      }
                    }
                  ?>
                ...
                <?php
                    ...
                    if(mysqli_num_rows($query_run) > 0) {
                      foreach($query_run as $items) {
                        ?>
                        <li id="myTable">
                          <a href='view-archived.php?id=<?php echo $items['id']?> &lastname=<?php echo $items['lastname']?> &firstname=<?php echo $items['firstname']?> &middlename=<?php echo $items['middlename']?> &gender=<?php echo $items['gender']?> &suffix=<?php echo $items['suffix']?> &dob=<?php echo $items['dob'] ?>&age=<?php echo $items['age']?> &contact=<?php echo $items['contact'] ?>&email=<?php echo $items['email']?> &address=<?php echo $items['address']?> &barangay=<?php echo $items['barangay']?> &img=<?php echo $items['img'] ?>'><?= ucwords($items['lastname']), ' ', ucwords($items['firstname']) ?></a>
                        </li>
                        <?php
                      }
                    }
